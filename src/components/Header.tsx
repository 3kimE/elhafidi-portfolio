...
const Header: React.FC = () => {
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center justify-between">
          <div className="flex-shrink-0">
            <a href="#home" className="flex items-center">
              <span className={`text-2xl font-extrabold ${isScrolled ? 'text-white' : 'text-white'}`}>
          ...
          <nav className="hidden md:flex space-x-8">
            {navLinks.map((link) => (
              <a 
                key={link.name} 
                href={link.href} 
                className={`font-medium transition-colors duration-300 hover:-translate-y-1 transform ${
      ...
      {/* Mobile Navigation */}
      {isMenuOpen && (
        <nav className="md:hidden bg-gray-900/95 backdrop-blur-lg shadow-2xl animate-fade-in">
